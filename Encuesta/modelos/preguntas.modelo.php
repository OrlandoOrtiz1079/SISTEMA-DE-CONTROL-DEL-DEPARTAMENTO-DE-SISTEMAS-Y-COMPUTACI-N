...
class ModeloPreguntas
{
    ...
    MOSTRAR DOCENTES
    =============================================*/
    public static function MdlMostrarRespuestas($tabla, $item, $valor)
    {

        if ($item != null) {
            $stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE fecha LIKE '$valor'");
    ...
    public static function MdlMostrarPregnutas($fecha, $item, $Comparacion)
    {
        $stmt51 = Conexion::conectar()->prepare("SELECT * FROM encuesta WHERE $item = :$item AND fecha LIKE '$fecha'");
        $stmt51->bindParam(":" . $item, $Comparacion, PDO::PARAM_STR);
        $stmt51->execute();
        echo $stmt51->rowCount();
        return $stmt51->rowCount();
    }
